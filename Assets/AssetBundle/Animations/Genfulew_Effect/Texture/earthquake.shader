...
Shader "EffekseerMaterial/earthquake" {
...
Properties{
	...
	_dis_Tex("Color (RGBA)", 2D) = "white" {}
_water_Tex("Color (RGBA)", 2D) = "white" {}
...
float4 val8 = ConvertFromSRGBTexture(tex2Dlod(_dis_Tex,float4(GetUV(uv1),0,0)));
float3 val3=val8.xyz;
float3 val9=(val3+float3(val1,val1,val1));
float4 val16 = ConvertFromSRGBTexture(tex2Dlod(_water_Tex,float4(GetUV(uv1),0,0)));
float val15=val16.w;
float3 val11=val16.xyz;
float3 val17= vcolor.xyz;
float3 val22=(val11*val17);
float3 normalDir = float3(0.5,0.5,1.0);
float3 worldPositionOffset = float3(0.0,0.0,0.0);
float3 baseColor = float3(0.0,0.0,0.0);
float3 emissive = val22;
float metallic = float(0.5);
float roughness = float(0.5);
float ambientOcclusion = float(1.0);
float opacity = val15;
float opacityMask = val9.x;
...
float val0=customData1.x;
float val1=(val0-float(1.0));
float4 val8 = ConvertFromSRGBTexture(tex2D(_dis_Tex,GetUV(uv1)));
float3 val3=val8.xyz;
float3 val9=(val3+float3(val1,val1,val1));
float4 val16 = ConvertFromSRGBTexture(tex2D(_water_Tex,GetUV(uv1)));
float val15=val16.w;
float3 val11=val16.xyz;
float3 val17= vcolor.xyz;
float3 val22=(val11*val17);
